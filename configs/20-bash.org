...
The XDG environment variables are taken care of by the Guix setup-environment script. So we only need to generate these lines when there is no Guix installed:

#+begin_src bash :mkdirp yes :tangle (if (executable-find "guix") "no" "home/.profile")
...
Put the above named block into .profile:

#+begin_src bash :noweb yes :noweb-ref profile
