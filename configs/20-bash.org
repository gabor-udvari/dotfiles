...
#+begin_src bash :mkdirp yes :tangle home/.local/bin/youtube-rss-get.sh :tangle-mode (identity #o755)
  ...
  # Somehow I cannot match the youtube channel ids with an underscore like this:
  # channel_id="$(curl -s "$1" | sed -n 's#^.*\(UC[-_a-zA-Z0-9]\{22\}\)".*$#\1#p')"
  # This works if there is an about page:
  channel_id="$(curl -s "$1" | sed -n 's#^.*\(UC.\{22\}\)/about.*$#\1#p')"
